...
description: 用户询问如何开始 v3.0 训练、正在模块一至三中卡住，或希望复盘训练过程时使用。提供最低门槛下一步，不评分、不催促、不替用户判定完成。
---
...
首次训练请求先确认用户是第一次接触还是已有具体工具/模块材料；不要直接展开完整路径。只根据用户已提供的练习材料判断一个下一步；不要给阶段评分，不把训练变成任务清单。急性状态或超出边界时退出本路线。
