---
name: reaction-mode
description: 反应模式工作台总入口。用户带着具体困扰、情绪、训练问题、关系互动或不确定从哪里开始时使用；先做安全与场景判断，再以咨询式、逐步递进的会谈推进一个最合适的主要路线。
---
...
- 实用支持和情感支持：读取对应 runbook；不要自动进入模式分析。
- 急性峰值或身体稳定：读取 `references/knowledge/runbooks/acute_trigger.md`、`references/knowledge/runbooks/body_regulation.md` 或 `references/knowledge/runbooks/transfer_out.md`，只执行其边界。
- 明确模式探索：读取 `references/knowledge/runbooks/inner_pattern_work.md`，并按《递进式会谈编排》开始。
- 训练、建设、日志或体系说明：读取对应完整 runbook 和所需知识；探索性对话仍按《递进式会谈编排》推进。
