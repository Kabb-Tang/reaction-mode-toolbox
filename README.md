...
安装后，优先直接描述你的处境或需求；`reaction-mode` 是默认总入口，会先判断安全与场景，再在同一轮完成最合适的主要路线。也可以在支持原生多 Skill 的环境中直接选用任一叶子 Skill。
...
优先把用户的自然语言请求交给 `reaction-mode`。它在同一轮选择一个主要路线并完成回应；不要求平台支持斜杠命令、子 Skill 调用、脚本或本地文件系统。
