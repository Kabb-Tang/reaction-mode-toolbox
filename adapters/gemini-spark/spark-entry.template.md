...
- `core/focus-lifecycle.md`
- `core/progressive-session.md`
- `core/safety-boundary.md`
- `core/output-self-check.md`
- `modules/router.md`

先判断当前会谈焦点是继续、重新聚焦还是收束，再根据路由选择一个主要模块。探索类请求按《递进式会谈编排》逐步推进：先定位，再展开，再做暂定观察，最后才小剂量推进；不要因已能分类就一次讲完答案，也不要在焦点完成后主动安排下一模块。不要要求用户重新发送提示词，也不要让用户在多个模块之间自行选择。
